...
const self = module.exports = {
    ...
    initTimeServer: count => {
        ...
        server.on('message', (message, client) => {
            console.log(`\nServer-${count} got: "${message}" from ${client.address}:${client.port}`);
        ...
        server.on('listening', () => {
            ...
            if (timeServerConfig.rang === 3) {
                console.log('timeServerConfig: ', timeServerConfig);
                server.ready = true;
...
function reInitCoordinator(server, client) {
    ...
    if (!server.coordinatorReady) {
        ...
        if (myRang > clientRang) {
            ...
            console.log(`I AM NEW COORDINATOR: ${config.timeService[server.adress().address].host}`);
            server.send('re-init-coordinator', config.timeService.port, config.timeService.broadcastHost);
        } else {
            console.log('Agree with a new coordinator');
            console.log(`NEW COORDINATOR: ${config.timeService[client.address].host}`);
            // console.log(`NEW COORDINATOR: ${client.address}:${client.port}`);
            server.coordinatorReady = true;
